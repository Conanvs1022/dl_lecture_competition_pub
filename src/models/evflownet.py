...
class EVFlowNet(nn.Module):
    def __init__(self, args):
        ...
        self.encoder1 = general_conv2d(in_channels = 4, 
                                       out_channels=_BASE_CHANNELS, 
                                       do_batch_norm=not self._args.no_batch_norm,
                                       dropout_prob=0.1)
        self.encoder2 = general_conv2d(in_channels = _BASE_CHANNELS, 
                                       out_channels=2*_BASE_CHANNELS, 
                                       do_batch_norm=not self._args.no_batch_norm,
                                       dropout_prob=0.1)
        self.encoder3 = general_conv2d(in_channels = 2*_BASE_CHANNELS, 
                                       out_channels=4*_BASE_CHANNELS, 
                                       do_batch_norm=not self._args.no_batch_norm,
                                       dropout_prob=0.1)
        self.encoder4 = general_conv2d(in_channels = 4*_BASE_CHANNELS, 
        ...
        self.resnet_block = nn.Sequential(
            *[build_resnet_block(8*_BASE_CHANNELS, do_batch_norm= not self._args.no_batch_norm, dropout_rate=0.1) for i in range(2)]
        )
